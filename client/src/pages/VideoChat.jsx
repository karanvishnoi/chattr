  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-dark overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
        {/* ========== DESKTOP: LEFT (videos) ========== */}
        {/* ========== MOBILE: FULL (single video stack, local overlay) ========== */}
        <div className={`relative bg-dark md:w-[42%] lg:w-[38%] md:p-3 ${
          showMobileChat ? 'hidden md:block' : 'flex-1 md:flex-none'
        }`}>
          {/* Desktop: Two videos stacked with aspect ratio */}
          <div className="hidden md:flex flex-col gap-3 h-full justify-center">
            ...
            <div className="relative bg-dark-card rounded-xl overflow-hidden border border-dark-border" style={{ aspectRatio: '4/3' }}>
              ...
              {connectionState !== 'connected' && <VideoPlaceholder status={status} />}
          ...
          {/* Mobile: Remote fullscreen + Local PIP */}
          <div className="md:hidden h-full relative">
            ...
            <div className="absolute bottom-3 left-3 flex gap-2 z-10">
              <button onClick={toggleMute} className={`w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-sm transition-colors cursor-pointer ${isMuted ? 'bg-danger/80 text-white' : 'bg-dark/70 text-white hover:bg-dark'}`}>
                {isMuted ? (
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2c0 .76-.13 1.49-.36 2.18"/></svg>
                ) : (
              ...
              <button onClick={toggleCamera} className={`w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-sm transition-colors cursor-pointer ${isCameraOff ? 'bg-danger/80 text-white' : 'bg-dark/70 text-white hover:bg-dark'}`}>
                {isCameraOff ? (
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="1" y1="1" x2="23" y2="23"/><path d="M21 21H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h3m3-3h6l2 3h4a2 2 0 0 1 2 2v9.34m-7.72-2.06a4 4 0 1 1-5.56-5.56"/></svg>
                ) : (
        ...
        {/* RIGHT: Chat panel */}
        <div className={`flex-1 flex flex-col border-l border-dark-border min-h-0 ${showMobileChat ? 'flex' : 'hidden md:flex'}`}>
...
function LocalVideoContent({
  localVideoRef, isCameraOff, isMuted, toggleCamera, toggleMute,
  showDeviceMenu, setShowDeviceMenu, videoDevices, audioDevices,
  currentVideoId, currentAudioId, switchVideoDevice, switchAudioDevice, myCountry,
}) {
  return (
    <>
      ...
      <div className="absolute bottom-2 right-2 flex gap-1.5">
        <button onClick={toggleMute} className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm transition-colors cursor-pointer ${isMuted ? 'bg-danger/80 text-white' : 'bg-dark/70 text-white hover:bg-dark'}`}>
          {isMuted ? (
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2c0 .76-.13 1.49-.36 2.18"/></svg>
          ) : (
